...
/*
-- Request Params --
- code -- This is the actual code that can be used with the Spotify SDK to get the user's refresh token
- state -- This is the random string that we generated in the "link" route. It is used to figure out which user to 
           associate these tokens with.
...
async function handleOauthCallback(req, res) {
    try {
        ...
        const user_email = await spotifyStateRef.get().then((doc) => {
        ...
        Spotify.authorizationCodeGrant(code, (error, data) => {
            ...
            Spotify.getMe(async(error, userResults) => {
                ...
                // We have a Spotify refresh token and the user identity now.
                const refreshToken = data.body['refresh_token'];

                // Get spotify data and store in the user's document
                db.collection("user_metadata").doc(user_email).update({
...
/*
-- Request Property --
  - user_spotify_refresh_token -- Requires the refresh_token so the Spotify API knows whose currently-playing data we are requesting.
...
async function handleCurrentlyPlaying(req, res) {
    try {
        ...
        const song = await Spotify.getMyCurrentPlayingTrack().then((data, err) => {
            return data.body.item;
        });
        res.status(200).json({ song });
    } catch (e) {
        console.log(e);
        res.status(500).json({ message: "Something went wrong while fetching the data" });
    }
}

/*
-- Request Property --
  - user_email -- Requires the request.user_email property to be set so we know which user to fetch token for. This is set in the authMiddleware.
...
async function spotifyTokenMiddleware(req, res, next) {
    ...
    try {
        req.user_spotify_refresh_token = await db.collection("user_metadata").doc(user_email).get().then((doc) => doc.data().spotify_refresh_token);
